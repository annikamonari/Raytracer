...
"""Create a ray with an initial point and direction"""
	...
	def parallaxtrace(self, ray, sphericalrefraction, outputplane):
		"""estimates the parallaxial focus through 1 spherical
		refraction lens using the trace() method for two rays"""
	...
	def parallaxtrace2(self, sphericalrefraction1, sphericalrefraction2, outputplane, ray):
		"""estimates the parallaxial focus through two sphericalrefraction lenses 
...
"""Create a collimated beam from an input ray, a beam radius and the 
number of radii within that radius"""
	...
	def __repr__(self):
		return "Collimated Beam of radius = %s, density = %s, centred at (%s,%s)" % (self._r,self.d,self._x,self._y)

	def create(self):
		for r in np.linspace(0, self._r, self._n, endpoint = True):
			for i in np.linspace(0, 2*np.pi, (2*np.pi*r*(self._n-1))/self._r, endpoint = True):
	...
	def trace(self, sphericalrefraction, outputplane):
		...
		self.plot()
		self.spotplot(outputplane.zoutput)
		print 'Diffraction Limit = %s' %(((outputplane.zoutput - sphericalrefraction.z0)*680*10**(-9))/(2*self._r))
	...
	def trace2(self, sphericalrefraction1, sphericalrefraction2, outputplane):
		...
		self.plot()
		self.spotplot(outputplane.zoutput)
		print 'Diffraction Limit = %s' %(((outputplane.zoutput - sphericalrefraction1.z0)*680*10**(-9))/(2*self._r))
